...
//Part One code, I talked with Jesse about mmap function parameters
//Part Two offset, I talked with Drake and Nic about why file[10] doesn't work
//rgb_to_pixel and pixel_to_rgb used previous work

//All, bugged Kathy about basically everything
...
void bitmap_mirror(struct bitmap *bmp)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            p[i] = bmp->pixels[i]; //this is for the original image
            //p[x] = bmp->pixels[x]; for row y=0

            //int row = (y+1) * nWidth;
            //int j = row - 1 -x;
            p[j] = bmp->pixels[i]; //this is for the mirrored image
